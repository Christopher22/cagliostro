...
cagliostro::model::Page *cagliostro::model::Config::parse(cagliostro::model::Wizard *parent) {
  ...
  this->parse("page", [&]() {
	if (xml_.name() == "title") {
	  ...
	  } else if (this->parse(xml_.readElementText(), page) == nullptr) {
...
cagliostro::model::content::Content *cagliostro::model::Config::parse(const QString &path, Page *parent) {
  QUrl url(path);
  if (url.isLocalFile() || !path.contains("://")) {
	// Make files absolute regarding the root directory.
	QFileInfo info(!url.isLocalFile() ? path : url.toLocalFile());
	if (info.isRelative()) {
	  info = QFileInfo(root_.filePath(info.filePath()));
	}

	// Ensure files do exist.
	if (!info.exists()) {
	  return nullptr;
	}
  ...
  return content::Video::load(url, parent);
}
